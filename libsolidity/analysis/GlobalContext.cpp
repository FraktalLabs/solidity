...
int magicVariableToID(std::string const& _name)
{
	...
	else if (_name == "xyield") return -32;
	else if (_name == "xchancreate") return -33;
	else
...
inline vector<shared_ptr<MagicVariableDeclaration const>> constructMagicVariables()
{
	...
	return {
		...
		magicVarDecl("xyield", TypeProvider::function(strings{}, strings{}, FunctionType::Kind::XYield, StateMutability::Pure)),
		magicVarDecl("xchancreate", TypeProvider::function(strings{"uint8"}, strings{"xchannel"}, FunctionType::Kind::XChanCreate)),
	};
}
